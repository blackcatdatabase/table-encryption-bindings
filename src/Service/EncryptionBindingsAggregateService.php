...
namespace BlackCat\Database\Packages\EncryptionBindings\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\EncryptionBindings\Dto\EncryptionBindingDto;
use BlackCat\Database\Packages\EncryptionBindings\Mapper\EncryptionBindingDtoMapper;
 ...
 */
final class EncryptionBindingsAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private EncryptionBindingRepository $encryptionBindingRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'encryption_bindings' => $this->encryptionBindingRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\EncryptionBindings\ModuleException("Record {$idStr} not found for locking.");
            }
