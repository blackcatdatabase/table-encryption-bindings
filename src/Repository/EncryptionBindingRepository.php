...
use BlackCat\Database\Packages\EncryptionBindings\Repository\EncryptionBindingRepositoryInterface;
...
class EncryptionBindingRepository implements EncryptionBindingRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\EncryptionBindings\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'key_wrapper_id' ]);
          return count($rows);
        }
    ...
    public function getByEntityTableAndEntityPkAndFieldNameNorm(string $entityTable, string $entityPk, string $fieldNameNorm, bool $asDto = false): array|\BlackCat\Database\Packages\EncryptionBindings\Dto\EncryptionBindingDto|null {
        return $this->getByUnique([ 'entity_table' => $entityTable, 'entity_pk' => $entityPk, 'field_name_norm' => $fieldNameNorm ], $asDto);
    }
    ...
    public function getIdByEntityTableAndEntityPkAndFieldNameNorm(string $entityTable, string $entityPk, string $fieldNameNorm) {
        $row = $this->getByEntityTableAndEntityPkAndFieldNameNorm($entityTable, $entityPk, $fieldNameNorm, false);
