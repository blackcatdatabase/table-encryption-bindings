...
namespace BlackCat\Database\Packages\EncryptionBindings\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\EncryptionBindings\Definitions;
use BlackCat\Database\Packages\EncryptionBindings\Criteria;
use BlackCat\Database\Packages\EncryptionBindings\Dto\EncryptionBindingDto as Dto;
use BlackCat\Database\Packages\EncryptionBindings\Mapper\EncryptionBindingDtoMapper as RowMapper;
...
final class EncryptionBindingRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\EncryptionBindings\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'entity_table', 'entity_pk', 'field_name' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\EncryptionBindings\Definitions::table());
            return $bulk->upsertMany($rows, [ 'entity_table', 'entity_pk', 'field_name' ], [ 'key_wrapper_id' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\EncryptionBindings\Dto\EncryptionBindingDto|null */
    public function getByEntityTableAndEntityPkAndFieldNameNorm(string $entityTable, string $entityPk, string $fieldNameNorm, bool $asDto = false): array|\BlackCat\Database\Packages\EncryptionBindings\Dto\EncryptionBindingDto|null {
        $row = $this->getByUnique([ 'entity_table' => $entityTable, 'entity_pk' => $entityPk, 'field_name_norm' => $fieldNameNorm ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\EncryptionBindings\Mapper\EncryptionBindingDtoMapper::fromRow($row);
    }
    public function existsByEntityTableAndEntityPkAndFieldNameNorm(string $entityTable, string $entityPk, string $fieldNameNorm): bool {
        $where = 't.' . Ident::q($this->db, 'entity_table') . ' = :uniq_entity_table' . ' AND ' . 't.' . Ident::q($this->db, 'entity_pk') . ' = :uniq_entity_pk' . ' AND ' . 't.' . Ident::q($this->db, 'field_name_norm') . ' = :uniq_field_name_norm';
        return $this->exists($where, [ 'uniq_entity_table' => $entityTable, 'uniq_entity_pk' => $entityPk, 'uniq_field_name_norm' => $fieldNameNorm ]);
    }
    /** @return int|string|null */
    public function getIdByEntityTableAndEntityPkAndFieldNameNorm(string $entityTable, string $entityPk, string $fieldNameNorm) {
        $row = $this->getByEntityTableAndEntityPkAndFieldNameNorm($entityTable, $entityPk, $fieldNameNorm);
        return $row ? ($row['id'] ?? null) : null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\EncryptionBindings\Dto\EncryptionBindingDto|null */
    public function getByEntityTableAndEntityPkAndFieldName(string $entityTable, string $entityPk, string $fieldName, bool $asDto = false): array|\BlackCat\Database\Packages\EncryptionBindings\Dto\EncryptionBindingDto|null {
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\EncryptionBindings\Dto\EncryptionBindingDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\EncryptionBindings\Dto\EncryptionBindingDto|null {
